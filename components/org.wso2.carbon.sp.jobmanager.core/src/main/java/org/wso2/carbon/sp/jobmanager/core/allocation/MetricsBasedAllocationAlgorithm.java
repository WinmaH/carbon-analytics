    ...
    public void retrieveData(DistributedSiddhiQuery distributedSiddhiQuery,
        ...
        try {
            for (SiddhiAppHolder appHolder : appsToDeploy) {
                ...
                logger.info("---------------------------------------------------");
                String query = "SELECT m7,m14,m15,m16 FROM metricstable where exec=" +
                ...
                if ( resultSet.isBeforeFirst()){     //Check the corresponding partial siddhi app is having the metrics
                    ...
                    while(resultSet.next()) {
                        ...
                        double memoryUsage = (double) (resultSet.getLong("m14") - resultSet.getLong("m15"));
                        logger.info("Memory Usage :" + memoryUsage);

                        partailSiddhiApps.add(new PartialSiddhiApp(processCPU, memoryUsage, (1 / latency), appHolder.getAppName()));
                    ...
                    partailSiddhiApps.add(new PartialSiddhiApp(processCPU, memoryUsage, 0.0, appHolder.getAppName()));
    ...
    public void executeKnapsack(Map<String, ResourceNode> resourceNodeMap,
        ...
        if (deploymentConfig != null && !resourceNodeMap.isEmpty()) {
            if (resourceNodeMap.size() >= minResourceCount) {
                ...
                double TotalMemoryUsagePartialSiddhi = 0;
                ...
                for (int j=0 ;j< partialSiddhiApps.size() ;j++ ){
                    ...
                    TotalMemoryUsagePartialSiddhi = TotalMemoryUsagePartialSiddhi + partialSiddhiApps.get(j).getMemoryUsage();

                }
                logger.info("TotalCPUUsagePartialSiddhi : " + TotalCPUUsagePartialSiddhi + "\n" );
                logger.info("TotalMemoryUsagePartialSiddhi :" + TotalMemoryUsagePartialSiddhi + "\n" );

                for (int p=0; p< resourceNodeMap.size(); p++){
                    ResourceNode resourceNode = (ResourceNode) resourceIterator.next();
                    multipleKnapsack.addKnapsack(new Knapsack((TotalCPUUsagePartialSiddhi/ resourceNodeMap.size()), (TotalMemoryUsagePartialSiddhi/ resourceNodeMap.size()),
                            resourceNode));
                    logger.info("created a knapsack of " + resourceNode);
                    logger.info("Allocatable CPU usage & Memory usage for " +
                            resourceNode.toString() + " is : " +
                            TotalCPUUsagePartialSiddhi / multipleKnapsack.getKnapsacks().size()+" , "+TotalMemoryUsagePartialSiddhi / multipleKnapsack.getKnapsacks().size() + "\n");
                }
                int i=0;
                ...
                if (partialSiddhiApps.size() > 0) {
                   ...
                   // logger.info("multipleKnapsack.getKnapsacks().size() " + multipleKnapsack.getKnapsacks().size());
                    for (int g=0; g < multipleKnapsack.getKnapsacks().size() ; g++){
                        temp.add(multipleKnapsack.getKnapsacks().get(g).getCPUCapacity());
                        tempMemory.add(multipleKnapsack.getKnapsacks().get(g).getMemoryCapacity());

                        logger.info(multipleKnapsack.getKnapsacks().get(g).getCPUCapacity() + " added");
                        logger.info(multipleKnapsack.getKnapsacks().get(g).getMemoryCapacity() + " added");
                    }
                    Collections.sort(temp);
                    Collections.sort(tempMemory);
                    int k = 1;
                    for (PartialSiddhiApp item : partialSiddhiApps) {
                        ...
                        for(int h=0; h< multipleKnapsack.getKnapsacks().size(); h++) {
                            if (multipleKnapsack.getKnapsacks().get(h).getCPUCapacity() == s) {
