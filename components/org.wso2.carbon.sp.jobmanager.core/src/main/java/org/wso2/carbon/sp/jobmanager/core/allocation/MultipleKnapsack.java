...
public class MultipleKnapsack {
                ...
                for (int lKnapsack = 0; lKnapsack < knapsacks.size(); lKnapsack++) {
                    for (int lItem = 0; lItem < knapsacks.get(lKnapsack).getpartialSiddhiApps().size(); lItem++) {
                        ...
                        if(!globalKnapsack.equals(localKnapsack)) {
                            ...
                            if((globalpartialSiddhiApps.get(gItem).getcpuUsage() <= localpartialSiddhiApps.get(lItem).getcpuUsage() + localKnapsack.getCPUCapacity())
                                && (globalpartialSiddhiApps.get(gItem).getMemoryUsage() <= localpartialSiddhiApps.get(lItem).getMemoryUsage() + localKnapsack.getMemoryCapacity())){
                                ...
                                for(Knapsack knapsack : knapsacks) {
                                    if(knapsack.equals(localKnapsack)) {
                                        ...
                                        local.setCPUCapacity(localKnapsack.getCPUCapacity() + localpartialSiddhiApps.get(lItem).getcpuUsage() - globalpartialSiddhiApps.get(gItem).getcpuUsage());
                                        local.setMemoryCapacity(localKnapsack.getMemoryCapacity() + localpartialSiddhiApps.get(lItem).getMemoryUsage() - globalpartialSiddhiApps.get(gItem).getMemoryUsage());
                                        ...
                                        global.setCPUCapacity(global.getCPUCapacity() + global.getpartialSiddhiApps().get(gItem).getcpuUsage());
                                        global.setMemoryCapacity(global.getMemoryCapacity() + global.getpartialSiddhiApps().get(gItem).getMemoryUsage());
    ...
    public void shufflepartialSiddhiAppsInKnapsacks() {
        ...
        for(Knapsack knapsack : knapsacks) {
            ...
            for(Iterator<PartialSiddhiApp> it = partialSiddhiAppsInKnapsacks.iterator(); it.hasNext(); ) {
                ...
                if(item.getcpuUsage() <= knapsack.getCPUCapacity() && item.getMemoryUsage() <= knapsack.getMemoryCapacity()) {
    ...
    public void greedyMultipleKnapsack(LinkedList<PartialSiddhiApp> partialSiddhiApps) {
        ...
        double bestmemoryUsageDefference;
        double currentcpuUsageDifference;
        double currentmemoryUsageDifference;
        ...
        for (int i = 0; i < partialSiddhiApps.size(); i++) {
            ...
            bestcpuUsageDifference = bestmemoryUsageDefference = Integer.MAX_VALUE;
            bestKnapsack = null;
            for (int j = 0; j < knapsacks.size(); j++) {
                if(knapsacks.get(j).getCPUCapacity() >= partialSiddhiApps.get(i).getcpuUsage()
                        && knapsacks.get(j).getMemoryCapacity() >= partialSiddhiApps.get(i).getMemoryUsage()) {

                    currentcpuUsageDifference = knapsacks.get(j).getCPUCapacity() - partialSiddhiApps.get(i).getcpuUsage(); //remaining cpu capacity of the knapsack
                    currentmemoryUsageDifference = knapsacks.get(j).getMemoryCapacity() - partialSiddhiApps.get(i).getMemoryUsage();

                    if((currentcpuUsageDifference < bestcpuUsageDifference && currentcpuUsageDifference > 0)
                            && (currentmemoryUsageDifference < bestmemoryUsageDefference && currentmemoryUsageDifference >0)){

                        bestcpuUsageDifference = currentcpuUsageDifference;
                        bestmemoryUsageDefference =currentmemoryUsageDifference;
    ...
    public LinkedList<PartialSiddhiApp>  printResult(boolean flag) {
        for (Knapsack knapsack : knapsacks) {
            ...
            log.info("\nResourceNode\n" + "resourceNode: " + knapsack.getresourceNode()
                    ...
                    "\nTotal Usable Memory :" + knapsack.getStartmemoryUsage() +
                    "\nUsed CPU in this iteration : " + (knapsack.getStartcpuUsage() - knapsack.getCPUCapacity()) +
                    "\nUsed memory in this iteration : " + (knapsack.getStartmemoryUsage() - knapsack.getMemoryCapacity()) +
                    "\nRemaining CPU : " + knapsack.getCPUCapacity() +
                    "\nRemaining memory : " + knapsack.getMemoryCapacity() + "\n" );

            knapsack.setStartcpuUsage(knapsack.getCPUCapacity());
            knapsack.setStartmemoryUsage(knapsack.getMemoryCapacity());

            log.info("Initial CPU Usage & Memory Usage of " + knapsack.getresourceNode() + " are set to " + knapsack.getStartcpuUsage()+
                    " & " + knapsack.getStartmemoryUsage() + "\n");
            try {
                for (PartialSiddhiApp item : knapsack.getpartialSiddhiApps()) {
                    log.info("\nPartial siddhi app\n" + "Name: " + item.getName()
                            + "\nLatency : " + item.getlatency() + "\nCPU Usage : "
                            + item.getcpuUsage() + "\nMemory Usage : " + item.getMemoryUsage());
